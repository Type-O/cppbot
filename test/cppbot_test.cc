...
TEST(MarathonTest, BasicAssertions) {
	...
	Robot robot;
	robot.run_instructions(robot.parseInstructions("..\\..\\test\\Marathon.txt"), false);
	std::string s1 = robot.report(false);
	std::string s2 = "0,0,WEST";
	ASSERT_STREQ(robot.report(false).c_str(), s2.c_str());
}

TEST(TestInstructionSetOne, BasicAssertions) {

	Robot robot;
	robot.run_instructions(robot.parseInstructions("..\\..\\test\\IS1.txt"), false);
	std::string s1 = robot.report(false);
...
TEST(TestInstructionSetTwo, BasicAssertions) {
	...
	Robot robot;
	robot.run_instructions(robot.parseInstructions("..\\..\\test\\IS2.txt"), false);
	std::string s1 = robot.report(false);
	std::string s2 = "0,0,WEST";
	ASSERT_STREQ(robot.report(false).c_str(), s2.c_str());
}
...
TEST(TestInstructionSetThree, BasicAssertions) {
	...
	Robot robot;
	robot.run_instructions(robot.parseInstructions("..\\..\\test\\IS3.txt"),false);
	std::string s1 = robot.report(false);
...
TEST(MoveYWest, BasicAssertions) {
	...
	robot.run_instructions(list,false);
	std::string s1 = robot.report(false);
	std::string s2 = "0,0,WEST";
	list.pop_back();
	ASSERT_STREQ(robot.report(false).c_str(), s2.c_str());
...
TEST(BoundPlace, BasicAssertions) {
	...
	std::string s2 = robot.report(false);
	list.pop_back();
